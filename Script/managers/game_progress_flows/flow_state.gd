...
#定义scene_status的取值范围
#can_enter 解锁（可进入）、not_enter 未解锁（不可进入）、done 完成（可进入）
enum scene_status { can_enter, not_enter, done }  

#scene_status字典
var scene_states: Dictionary = {
	"res://scenes/scene1.tscn": scene_status.can_enter,
	"res://scenes/scene2.tscn": scene_status.not_enter,
}

#判断场景是否可进入，返回true表示可进入
func is_scene_unlocked(scene_path: String) -> bool:
